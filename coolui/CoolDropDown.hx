 ...
 *     CoolDropDown.makeStrIdLabelArray(["Option A", "Option B"]),
 *     function(id:String) { trace(id); }
 *   );
 *
 *   // With a custom header
 *   var dd = new CoolDropDown(x, y, data, cb,
 *     new CoolDropDown.DropDownHeader("Select...", new CoolDropDown.DropDownHeaderStyle())
 *   );
 *
 *   dd.selectedLabel   // text of the active option
 *   dd.selectedId      // active option ID
 *
...
class CoolDropDown extends FlxSpriteGroup {
	...
	/** Callback when selecting: `callback(id:String)`. */
	 ...
	 */
	public function new(px:Float = 0, py:Float = 0, data:Array<DropDownData>, ?callback:String->Void, ?header:DropDownHeader, _unused1:Dynamic = null,
			_unused2:Dynamic = null, width:Int = 120) {
		super(px, py);
		_data = data ?? [];
		_w = width;
...
private class _DropList extends FlxSpriteGroup {
	...
	function _build(T:CoolTheme):Void {
		...
		var p = bg.pixels;
		for (i in 0...bw) {
			p.setPixel32(i, 0, brdCol);
			p.setPixel32(i, bh - 1, brdCol);
		}
		for (j in 0...bh) {
			p.setPixel32(0, j, brdCol);
			p.setPixel32(bw - 1, j, brdCol);
		}
		bg.pixels = p;
