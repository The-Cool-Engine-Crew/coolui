...
class CoolTabMenu extends FlxSpriteGroup {
	...
	function _buildTabBtns(pw:Int, T:CoolTheme):Void {
		...
		_tabBtns = [];
		if (_tabDefs == null || _tabDefs.length == 0)
			return;
	...
	function _fadeBody():Void {
		...
		if (_bodyBg == null)
			return;
		_bodyBg.alpha = 0.55;
		_fadeTween = FlxTween.globalManager.tween(_bodyBg, {alpha: 1.0}, FADE_TIME, {ease: FlxEase.quartOut});
	}
...
private class _TabBtn extends FlxSpriteGroup {
	...
	public function new(bx:Float, by:Float, bw:Int, bh:Int, labelStr:String, name:String, T:CoolTheme) {
		super(bx, by);
		tabName = name;
		_bw = bw;
		_bh = bh;
