...
class CoolButton extends FlxSpriteGroup {
	...
	// ── Constructor ────────────────────────────────────────────────────────
	public function new(px:Float = 0, py:Float = 0, labelStr:String = "", ?onClick:Void->Void,
	                    width:Int = DEFAULT_W, height:Int = DEFAULT_H, style:Int = STYLE_DEFAULT) {
		super(px, py);
	...
	function set_enabled(v:Bool):Bool { _enabled = v; alpha = v ? 1.0 : 0.40; return v; }

	function get_btnWidth():Int  return _bw;
	function set_btnWidth(v:Int):Int  { if (v > 0 && v != _bw) resize(v, _bh); return _bw; }

	function get_btnHeight():Int return _bh;
	function set_btnHeight(v:Int):Int { if (v > 0 && v != _bh) resize(_bw, v); return _bh; }

	function get_label():String  return (_label != null) ? _label.text : "";
	...
	function _drawButton(s:FlxSprite, bgC:FlxColor, brdC:FlxColor):Void {
		...
		var p = s.pixels; var rad = CORNER_RADIUS;
		var hiC = FlxColor.fromRGB(
			Std.int(Math.min(255, bgC.red + 70)),
			Std.int(Math.min(255, bgC.green + 70)),
			Std.int(Math.min(255, bgC.blue + 70)));
		hiC.alphaFloat = 0.55;
		var shC = FlxColor.fromRGB(
			Std.int(Math.max(0, bgC.red - 35)),
			Std.int(Math.max(0, bgC.green - 35)),
			Std.int(Math.max(0, bgC.blue - 35)));
		shC.alphaFloat = 0.55;
		...
		for (py in 0...h) {
			...
			var rowC     = FlxColor.fromRGB(
				...
				Std.int(Math.max(0, Math.min(255, bgC.blue  + lift - drop))));
			for (px in 0...w) {
				if (_inCorner(px, py, w, h, rad))                              p.setPixel32(px, py, FlxColor.TRANSPARENT);
				else if (px == 0 || px == w-1 || py == 0 || py == h-1)        p.setPixel32(px, py, brdC);
				else if (py == 1 || px == 1)                                   p.setPixel32(px, py, hiC);
				else if (py == h-2 || px == w-2)                               p.setPixel32(px, py, shC);
				else                                                            p.setPixel32(px, py, rowC);
			}
		}
		s.pixels = p;
	}
	...
	inline function _inCorner(px:Int, py:Int, w:Int, h:Int, rad:Int):Bool {
		if (px < rad && py < rad)           { var dx = rad-1-px; var dy = rad-1-py; return (dx*dx+dy*dy) >= rad*rad; }
		if (px >= w-rad && py < rad)        { var dx = px-(w-rad); var dy = rad-1-py; return (dx*dx+dy*dy) >= rad*rad; }
		if (px < rad && py >= h-rad)        { var dx = rad-1-px; var dy = py-(h-rad); return (dx*dx+dy*dy) >= rad*rad; }
		if (px >= w-rad && py >= h-rad)     { var dx = px-(w-rad); var dy = py-(h-rad); return (dx*dx+dy*dy) >= rad*rad; }
		return false;
	}

	function _bgColor(T:CoolTheme):Int {
		return switch (_style) { case STYLE_ACCENT: T.accent; case STYLE_DANGER: T.error; case STYLE_GHOST: FlxColor.TRANSPARENT; default: T.bgHover; };
	}
	function _borderColor(T:CoolTheme):Int {
		return switch (_style) { case STYLE_ACCENT: T.accent; case STYLE_DANGER: T.error; default: T.borderColor; };
	}
	function _textColor(T:CoolTheme):Int {
		return switch (_style) { case STYLE_ACCENT: T.bgDark; case STYLE_DANGER: T.textPrimary; default: T.textPrimary; };
	}
	...
	function _flashPress():Void {
		_cancelTweens();
		_bg.alpha = 0.52; _hoverOverlay.alpha = 0.20;
	...
	function _cancelTweens():Void {
		...
		if (_tweenOvr != null) { _tweenOvr.cancel(); _tweenOvr = null; }
	}
