...
class CoolColorPicker extends FlxSpriteGroup {
	...
	function set_color(v:Int):Int {
		...
		_a = fc.alphaFloat;
		var r = fc.redFloat; var g = fc.greenFloat; var b = fc.blueFloat;
		_rgbToHsv(r, g, b);
		_refreshAll();
		return v;
	}
	...
	function _paintHueStrip():Void {
		...
		for (py in 0...h) {
			...
			for (px in 0...HUE_W) p.setPixel32(px, py, rgb | 0xFF000000);
		}
		_hueBitmap.pixels = p;
	}
	...
	function _paintAlphaStrip():Void {
		...
		for (px in 0...TOTAL_W) {
			...
			for (py in 0...ALPHA_H) p.setPixel32(px, py, (a << 24) | rgb | 0x80808080);
		}
	...
	function _paintPreview():Void {
		...
		var c = get_color();
		for (py in 0...PREVIEW_H) for (px in 0...TOTAL_W) p.setPixel32(px, py, c);
		_preview.pixels = p;
	}
	...
	function _syncHexField():Void {
		...
		_hexField.text = StringTools.hex(fc.red, 2) + StringTools.hex(fc.green, 2)
		                 + StringTools.hex(fc.blue, 2) + StringTools.hex(fc.alpha, 2);
	}
	...
	function _applyHex(s:String):Void {
		...
		try {
			if (len == 6)      parsed = Std.parseInt("0xFF" + s);
			else if (len == 8) parsed = Std.parseInt("0x" + s.substr(6, 2) + s.substr(0, 6));
		} catch (_) {}
		if (parsed != null) color = parsed;
	}

	function _fireChange():Void {
		if (onChange != null) onChange(get_color());
	}
	...
	override public function update(elapsed:Float):Void {
		...
		var mx    = mp.x - x; var my = mp.y - y;
		mp.put();

		// SV square drag
		var inSV = mx >= 0 && mx < SV_SIZE && my >= _svY && my < _svY + SV_SIZE;
		if (inSV && FlxG.mouse.justPressed) _draggingSV = true;
		if (!FlxG.mouse.pressed) _draggingSV = false;
		if (_draggingSV) {
			_s = Math.max(0, Math.min(1, mx / (SV_SIZE - 1)));
			_v = Math.max(0, Math.min(1, 1 - (my - _svY) / (SV_SIZE - 1)));
			_refreshAll(); _fireChange();
		}

		// Hue strip drag
		var inHue = mx >= SV_SIZE + GAP && mx < SV_SIZE + GAP + HUE_W && my >= _hueY && my < _hueY + SV_SIZE;
		if (inHue && FlxG.mouse.justPressed) _draggingHue = true;
		if (!FlxG.mouse.pressed) _draggingHue = false;
		if (_draggingHue) {
			_h = (1 - Math.max(0, Math.min(1, (my - _hueY) / (SV_SIZE - 1)))) * 360;
			_paintSVSquare(); _syncThumbs(); _paintAlphaStrip(); _paintPreview(); _syncHexField(); _fireChange();
		}

		// Alpha strip drag
		var inAlpha = mx >= 0 && mx < TOTAL_W && my >= _alphaY && my < _alphaY + ALPHA_H;
		if (inAlpha && FlxG.mouse.justPressed) _draggingAlpha = true;
		if (!FlxG.mouse.pressed) _draggingAlpha = false;
		if (_draggingAlpha) {
			_a = Math.max(0, Math.min(1, mx / (TOTAL_W - 1)));
			_paintPreview(); _syncThumbs(); _syncHexField(); _fireChange();
		}
	...
	function _rgbToHsv(r:Float, g:Float, b:Float):Void {
		...
		if (delta == 0) { _h = 0; return; }
		if (max == r)      _h = 60 * (((g - b) / delta) % 6);
		else if (max == g) _h = 60 * ((b - r) / delta + 2);
		else               _h = 60 * ((r - g) / delta + 4);
		if (_h < 0) _h += 360;
	}
