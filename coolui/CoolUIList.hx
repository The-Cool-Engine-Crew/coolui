...
class CoolUIList extends FlxSpriteGroup {
	...
	public function setItems(items:Array<String>):Void {
		_items = items ?? [];
		_scroll = 0;
		if (_selIdx >= _items.length)
			_selIdx = _items.length - 1;
		_rebuildRows();
		_updateScrollBar();
	}
	...
	function _build():Void {
		...
		for (i in 0..._w) {
			...
			p.setPixel32(i, _h - 1, brdC);
		}
		for (j in 0..._h) {
			p.setPixel32(0, j, brdC);
			p.setPixel32(_w - 1, j, brdC);
		}
		_bg.pixels = p;
		add(_bg);
	...
	function _rebuildRows():Void {
		...
		for (i in 0..._visRows) {
			...
			row.onClick = function(ri:Int) {
				var now = flixel.util.FlxTimer.globalManager != null ? haxe.Timer.stamp() : 0.0;
				var dbl = (now - _lastClick) < DCLICK_TIME;
				_lastClick = now;

				selectedIndex = ri;
				if (onSelect != null)
					onSelect(ri, _items[ri]);
				if (dbl && onDoubleClick != null)
					onDoubleClick(ri, _items[ri]);
				_focused = true;
			};
			_rows.push(row);
			add(row);
	...
	function _ensureVisible(idx:Int):Void {
		...
		if (idx < _scroll)
			_scroll = idx;
		if (idx >= _scroll + _visRows)
			_scroll = idx - _visRows + 1;
